window.addEventListener("load", function () {
  ...
  `;
    ...
    for (let i = 0; i < data.length; i++) {
      const obj = data[i];
      for (let j = 0; j < obj.데이터.length; j++) {
        let tag = `
              <div class="swiper-slide">
                <a href="${obj.데이터[j].링크}" class="ticket_slide_item">
                  <div class="ticket_image">
                    <img
                      src="${obj.데이터[j].이미지}"
                      alt="${obj.데이터[j].alt}"
                    />
                    <div class="ticket_rank">${obj.데이터[j].랭크}</div>
                  </div>
                  <div class="ticket_info">
                    <p class="ticket_item_title">${obj.데이터[j].타이틀}
                    </p>
                    <p class="ticket_place">${obj.데이터[j].장소}
                    </p>
                    <p class="ticket_day">${obj.데이터[j].날짜}</p>
                    <div class="ticket_option">
                    `;
        for (let k = 0; k < obj.데이터[j].옵션.length; k++) {
          tag += ` <span class="ticket_${obj.데이터[j].옵션[k].스타일}">${obj.데이터[j].옵션[k].텍스트}</span>`;
        }
        tag += `
                    </div>
                  </div>
